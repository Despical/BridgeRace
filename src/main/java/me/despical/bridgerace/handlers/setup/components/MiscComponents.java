...
import me.despical.bridgerace.utils.conversation.SimpleConversationBuilder;
	...
	public void registerComponent(SetupInventory setupInventory, StaticPane pane) {
		...
		pane.addItem(new GuiItem(bungeeItem, e -> {
			...
			if (location.distance(e.getWhoClicked().getWorld().getSpawnLocation()) <= Bukkit.getServer().getSpawnRadius()) {
				e.getWhoClicked().sendMessage(plugin.getChatManager().colorRawMessage("&c&l✖ &cWarning | Server spawn protection is &6" + Bukkit.getServer().getSpawnRadius() + " &cand sign you want to place is in radius of this protection! &c&lNon opped players won't be able to interact with this sign and can't join the game so."));
				return;
			}
			...
			List<String> locs = config.getStringList(path + "signs");
			locs.add(signLoc);

			config.set(path + "signs", locs);
		...
		pane.addItem(new GuiItem(new ItemBuilder(Material.NAME_TAG)
			...
			.lore("", "&aCurrently: &e" + config.getString(path + "mapname"))
			.build(), e -> {
			e.getWhoClicked().closeInventory();
		...
		pane.addItem(new GuiItem(new ItemBuilder(XMaterial.FILLED_MAP.parseItem())
			...
			e.getWhoClicked().closeInventory();
			player.sendMessage(plugin.getChatManager().getPrefix() + plugin.getChatManager().colorRawMessage("&7Check out our wiki: https://github.com/Despical/BridgeRace/wiki"));
		}), 7, 2);
	}
}