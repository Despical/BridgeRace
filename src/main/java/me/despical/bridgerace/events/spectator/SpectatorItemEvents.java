	...
	private void openSpectatorMenu(World world, Player p) {
		...
		for (Player player : world.getPlayers()) {
			if (players.contains(player) && !plugin.getUserManager().getUser(player).isSpectator()) {
				...
				String score = plugin.getChatManager().colorMessage("In-Game.Spectator.Target-Player-Score", p).replace("%placed_blocks%", Integer.toString(StatsStorage.getUserStats(player, StatsStorage.StatisticType.LOCAL_PLACED_BLOCKS)).
					replace("%broken_blocks%", Integer.toString(StatsStorage.getUserStats(player, StatsStorage.StatisticType.LOCAL_BROKEN_BLOCKS))));
